...
func TestList(t *testing.T) {
	sql := "SELECT * FROM vtc_heartbeat_project order by id limit 5"
	for _, v := range List(sql, "project") {
		fmt.Println(v)

	}
	fmt.Print(".................")
	fmt.Println(Get(sql, "project"))

	Insert("insert into vtc_heartbeat_project(`name`,`area`,`desc`,`api_key`,`queue`,`callback`,`created`,`modified`) values (?,?,?,?,?,?,?,?)", "project", []interface{}{"test", "test", "test", "test", "test", "test", "0000-00-00 00:00:00", "0000-00-00 00:00:00"})
}
